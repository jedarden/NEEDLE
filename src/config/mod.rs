...
///
/// A tuple of:
/// - `EnvGuard`: holds the crate-wide env lock and restores PATH when dropped
/// - `TempDir`: the temporary directory for test binaries
pub(crate) fn isolate_bead_cli_env() -> (crate::util::test_env::EnvGuard, tempfile::TempDir) {
...
/// Fleet backlog replenishment policy for work-generating strands.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GenerationConfig {
    /// Enable low-water generation after Pluck and Explore find no eligible work.
    #[serde(default = "GenerationConfig::default_enabled")]
    pub enabled: bool,

    /// Minimum eligible-ready reserve below which generators may bypass their
    /// ordinary cooldown. A value of zero disables low-water overrides.
    #[serde(default = "GenerationConfig::default_low_water_reserve")]
    pub low_water_reserve: usize,

    /// Duration of the workspace-plus-strand generation lease. The lease also
    /// bounds retry frequency after a generator finds no new work.
    #[serde(default = "GenerationConfig::default_lease_ttl_secs")]
...
impl Default for GenerationConfig {
    fn default() -> Self {
        Self {
            enabled: Self::default_enabled(),
            low_water_reserve: Self::default_low_water_reserve(),
...
fn validate_generation_field(field: &str, key_path: &str) -> Result<(), ConfigError> {
    let valid_fields = ["enabled", "low_water_reserve", "lease_ttl_secs"];
    if valid_fields.contains(&field) {
        Ok(())
    } else {
        Err(ConfigError::invalid_segment(
...
impl ConfigLoader {
    ...
    pub fn validate(config: &Config) -> Vec<ConfigError> {
        ...
        if config.strands.generation.enabled
            && config.strands.generation.low_water_reserve > 0
            && config.strands.generation.lease_ttl_secs == 0
        {
            errors.push(ConfigError::new(
                "strands.generation.lease_ttl_secs".to_string(),
                "must be at least 1 when low-water generation is enabled".to_string(),
            ));
        }
...
mod config_tests {
    ...
    fn lock_supervisor_env() -> crate::util::test_env::EnvGuard {
...
"#;
    ...
    #[test]
    fn workspace_config_overrides_low_water_generation() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(
            dir.path().join(".needle.yaml"),
            "strands:\n  generation:\n    enabled: true\n    low_water_reserve: 4\n    lease_ttl_secs: 600\n",
        ...
        assert_eq!(config.strands.generation.low_water_reserve, 4);
        assert_eq!(config.strands.generation.lease_ttl_secs, 600);
        assert!(sources.contains_key("strands.generation"));
    }
...
"#;
    ...
    fn supervisor_config_from_env_defaults() {
        let _env_guard = lock_supervisor_env();
...
"#;
    ...
    fn test_bead_cli_config_json_serialization_preserves_backend() {
        ...
        for backend in backends {
            ...
            assert_eq!(
                ...
                "Backend serialization failed for {:?}",
                backend
            );
        }
    }
    ...
    #[test]
    fn test_bead_cli_config_json_serialization_preserves_path() {
        let test_paths = vec![
            Some(PathBuf::from("/usr/local/bin/bead")),
            Some(PathBuf::from("~/.local/bin/bf")),
            Some(PathBuf::from("/custom/path/to/cli")),
            None,
        ];
    ...
    #[test]
    fn test_bead_cli_config_json_roundtrip_preserves_all_values() {
        let configs = vec![
            BeadCliConfig {
                backend: BeadBackend::Auto,
                path: None,
            ...
            BeadCliConfig {
                backend: BeadBackend::Bead,
                path: Some(PathBuf::from("/opt/bead/bin/bead")),
            },
        ];
        ...
        for original_config in configs {
            ...
            assert_eq!(
                deserialized.backend, original_config.backend,
                "Backend not preserved through JSON round-trip"
            );
            assert_eq!(
                deserialized.path, original_config.path,
                "Path not preserved through JSON round-trip"
            );
        }
    }

    #[test]
    fn test_bead_cli_config_yaml_roundtrip_preserves_all_values() {
        let configs = vec![
            BeadCliConfig {
                backend: BeadBackend::Auto,
                path: None,
            ...
            BeadCliConfig {
                backend: BeadBackend::Bead,
                path: Some(PathBuf::from("~/.local/bin/bead")),
            },
        ];
        ...
        for original_config in configs {
            ...
            assert_eq!(
                deserialized.backend, original_config.backend,
                "Backend not preserved through YAML round-trip"
            );
            assert_eq!(
                deserialized.path, original_config.path,
                "Path not preserved through YAML round-trip"
            );
        }
    }
    ...
    #[test]
    fn test_bead_cli_config_json_with_explicit_path_alias() {
        // Test that explicit_path alias works in JSON deserialization
        let json_with_explicit_path = r#"{"backend": "auto", "explicit_path": "/custom/path"}"#;

        let config: BeadCliConfig = serde_json::from_str(json_with_explicit_path).unwrap();

        assert_eq!(config.backend, BeadBackend::Auto);
        assert_eq!(
            config.path,
            Some(PathBuf::from("/custom/path")),
            "explicit_path alias should deserialize to path field"
        );
    }

    #[test]
    fn test_bead_cli_config_yaml_with_explicit_path_alias() {
        // Test that explicit_path alias works in YAML deserialization
        let yaml_with_explicit_path = "backend: auto\nexplicit_path: /custom/path";

        let config: BeadCliConfig = serde_yaml::from_str(yaml_with_explicit_path).unwrap();
    ...
    #[test]
    fn test_bead_cli_config_json_serialization_omits_none_path() {
        let config = BeadCliConfig {
            backend: BeadBackend::Bead,
            path: None,
        };
    ...
    #[test]
    fn test_bead_cli_config_yaml_serialization_omits_none_path() {
        let config = BeadCliConfig {
            backend: BeadBackend::Bead,
            path: None,
        };
        ...
        // When path is None, it should not appear in YAML output
        assert!(
            !yaml.contains("path:"),
            "None path should not appear in YAML output"
        );
    }
    ...
    fn test_bead_cli_config_all_backend_variants_json_serialization() {
        ...
        for backend in backends {
            let config = BeadCliConfig {
                backend: backend.clone(),
                path: Some(PathBuf::from("/usr/local/bin/cli")),
            };
            ...
            assert_eq!(
                deserialized.backend, backend,
                "Backend variant {:?} not preserved through JSON round-trip",
                backend
            );
            assert_eq!(
                deserialized.path,
    ...
    fn test_bead_cli_config_all_backend_variants_yaml_serialization() {
        ...
        for backend in backends {
            ...
            assert_eq!(
                deserialized.backend, backend,
                "Backend variant {:?} not preserved through YAML round-trip",
                backend
            );
            assert_eq!(
                deserialized.path,
    ...
    #[test]
    fn test_bead_cli_config_json_minimal_serialization() {
    ...
    #[test]
    fn test_bead_cli_config_yaml_minimal_serialization() {
        // Test minimal config with only required fields
        let config = BeadCliConfig {
            backend: BeadBackend::Auto,
            path: None,
    ...
    fn test_isolate_bead_cli_env_creates_temp_dir() {
        let (_guard, tmp_dir) = isolate_bead_cli_env();
    ...
    fn test_isolate_bead_cli_env_multiple_calls_create_different_dirs() {
        ...
        let (first_path, _tmp_dir1) = {
            let (_guard1, tmp_dir1) = isolate_bead_cli_env();
            (tmp_dir1.path().to_path_buf(), tmp_dir1)
        };
        let (_guard2, tmp_dir2) = isolate_bead_cli_env();
