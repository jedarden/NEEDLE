...
async fn strand_waterfall_pluck_mend_explore_knot() {
    ...
    assert_eq!(
        runner.strand_names(),
        vec!["pluck", "mend", "explore", "weave", "unravel", "analyze", "pulse", "reflect", "splice", "knot"],
        "waterfall should be pluck → mend → explore → weave → unravel → analyze → pulse → reflect → splice → knot"
    );
}
