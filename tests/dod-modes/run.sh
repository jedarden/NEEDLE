...
# ── needle_slow_targets ──────────────────────────────────────────────────────
assert_lines "target table lists 22 names" 22 needle_slow_targets
...
SLOW_TARGET=""
assert_lines "default selection is all 21 cargo targets" 21 selected_cargo_targets
...
if [[ "$GOT_HARNESSES" == "$WANT_HARNESSES" ]]; then
  ok "Clippy reads the 20 declared test harness roots from Cargo.toml"
else
