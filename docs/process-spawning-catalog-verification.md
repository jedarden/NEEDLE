...
- The now-retired configurable-runner integration target - 5 invocations (bash scripts)
...
- The now-retired configurable-runner integration target
...
**Now-retired configurable-runner integration target** (5 bash invocations):
- Tested check aggregation
- Tested failure capture in stdout/stderr
- Tested JSON report structure
- Tested that all checks ran despite failures

These were exactly the kind of process-spawning integration tests the catalog
was supposed to identify. The target was later retired with the unused
configurable runner; these counts remain the report's 2026-08-29 snapshot.
...
4. **Add missing test suite documentation:**
   ...
   - Document the configurable-runner integration target (5 sites; since retired)
...
2. **Manual verification of key files:**
   ...
   - Read samples from the configurable-runner integration target (since retired)
