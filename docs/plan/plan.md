...
confirmation.
Revision 36 makes the shared-checkout anti-collision design normative (section
4.12 and Phase 20). A harness that supports and opts into the pre-write hook
...
| `config/gitleaks.toml`, `scripts/secret-scan.sh` and repository secret scans | The sanitizer embeds the vendored ruleset, including a narrow nonsecret release-digest allowance. The gate selects a supported pinned scanner past stale PATH shadows, passes this config explicitly, redacts findings, and records executable/config digests for staged and shipped-source scans | Verified in `c062b792`, `f81f2e20`, and `ca5623f4` |
...
- A stale gitleaks 8.21.2 binary still appears first on the interactive PATH,
  so direct ad hoc invocation is not release evidence. Authoritative commit
  and shipped-source gates go through `scripts/secret-scan.sh`, which selects
  the pinned Nix 8.30.1 artifact, requires the vendored minimum, and records
  scanner and config identity.
...
whose own `.needle.yaml` declares neither `gates:` nor legacy `verification:`
gets the built-in no-explicit-gate resolution. An explicit `gates: []` or
`verification: []` is still an opt-out. NEEDLE first lets the language-default
resolver produce a `default_*` gate; when it produces none, the shipped
**interim fallback gate** selects its verifier from the committed workspace in
this exact order:
...
so it judges committed state rather than the shared checkout. A command
failure is a verification failure; failure to create the extraction, spawn the
command, or finish before the timeout is an execution error and does not
become task-failure evidence. The limits are the host-level
`validation.outcome_timeout_seconds` (default `50` seconds) and
`validation.stderr_cap_bytes` (default `4096` bytes); the latter caps captured
verifier stderr in failure diagnostics.

If no verifier is selected, NEEDLE checks the extraction's source workspace
with `git status --porcelain --untracked-files=all` because the archive itself
has no `.git`. `.beads/` and `.needle-predispatch-sha` bookkeeping noise is
ignored. An empty status passes on the agent's exit code alone but emits one
counted `WARN` and the `gate.no_verifier` telemetry event (`reason:
not_detected`). Uncommitted work fails the `fallback_clean_tree` gate.

The fallback is armed by default, so every unguarded workspace without an
override is covered without a config change. The rollout count is an
inventory snapshot, not a hard-coded workspace list. The per-workspace opt-out
is:
...
value wins. When fallback resolution is reached, the dispatch log records
whether it is armed or opted out.

This built-in fallback is explicitly **INTERIM**. The
`needle-d1b2ee0d` runner is its planned replacement. In particular,
...
### 4.12 Shared-checkout file-contention markers (revision 36, 2026-09-15)
...
| N-T16 | `src/telemetry/`, `src/outcome/` | Emit one `attempt.resolved` ledger event per dispatch with a provisional attempt ID until N-T03 | versioned schema fixture; exactly one event per dispatch in the file sink across success/failure/timeout/crash | transition; worker-side event live since `78b2adce` with tokens, cost and provider (4.9); off-host sink still `declarat-399b667a`; row schema_version 2 carries the ADR-030 `decomposed` class and `costed` flag (N-T46, N-T47) |
...
| N-T46 | `src/outcome/`, `src/telemetry/`, `src/stats/`, `src/evidence_routing.rs` | `decomposed` resolution class excluded from every verified-credit consumer (ADR-030) | split fixture earns no verified credit; stats column; schema fixture | landed `47cada1a` (2026-09-15), not yet deployed; acceptance is `cargo test --lib nt46_` plus `cargo test --test integration_spawn nt46_`, because the five-target boundary in `tests/cargo-targets` rules out a dedicated test target; `needle-16657d72` closed |
| N-T47 | `src/dispatch/`, `src/trace/`, `src/outcome/`, `src/config/` | Streamed usage accounting so killed attempts are charged; `costed` flag; per-attempt spend cap resolving `budget_exhausted` (ADR-030) | killed-transcript replay carries cost; envelope reconciles; cap stops within one event with no failure count | accounting landed `645b3902` (2026-09-15), not yet deployed: usage is replayed from the captured stream at resolution, and feeding the accumulator live belongs to the cap; acceptance `cargo test --lib nt47_`; `needle-ec41ceba` closed. Spend cap still open, `needle-c0003de3` |
